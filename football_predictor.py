...
# ADVANCED FOOTBALL MATCH OUTCOME PREDICTOR
...
def _build_synthetic_dataset(self) -> List[MatchResult]:
    ...
    clubs.  Statistical parameters are calibrated to real PL distributions:
      ...
      - Shots: home 12–15, away 9–12
      - Possession: home 50–60 %
    """
    rng = np.random.default_rng(seed=42)

    teams = {
        # team_name: (attack_mu, defence_mu)  — controls goal-scoring tendency
        "Manchester City":    (2.30, 0.85),
        "Arsenal":            (2.10, 0.95),
        "Liverpool":          (2.20, 1.00),
        "Chelsea":            (1.80, 1.10),
        "Tottenham":          (1.85, 1.30),
...
where α = attack, β = defence (lower is better), γ = home advantage.
"""
...
def _compute_lambdas(self) -> Tuple[float, float]:
    ...
    xg_home = (self.home.avg_xg_for + (1 - self.away.avg_xg_against / self.league_h)) * self.HOME_ADVANTAGE
...
def _dixon_coles_correction(hg: int, ag: int, lam_h: float, lam_a: float,
    ...
    """
    Applies the Dixon-Coles (1997) low-score correction factor τ,
...
independent Poisson distributions calibrated to λ_home and λ_away.
...
def run(self) -> "MonteCarloSimulator":
    ...
    log.debug("Monte Carlo: %d simulations completed (λH=%.3f, λA=%.3f).",
...
def top_scores(self) -> List[Tuple[int, int, float]]:
    df = self._require_results()
    score_counts = df.groupby(["home_goals", "away_goals"]).size().reset_index(name="count")
...
Pipeline:
  ...
  3. For each fixture:
     ...
     b. Compute Poisson λ values                       (PoissonGoalModel)
...
@staticmethod
def save_json(reports: List[PredictionReport], path: str = "football_predictions.json") -> None:
